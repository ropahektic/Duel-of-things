...
async function deduplicateCarsByModel() {
  ...
  console.log('\nDeduplicating by make and model...');
  
  // Track seen combinations: make -> model -> first car
  const seen = new Map();
  const deduplicated = [];
  ...
  for (const car of cars) {
    ...
    // If we haven't seen this (make, model) combination, keep it
    if (!makeMap.has(model)) {
      makeMap.set(model, car);
  ...
  sortedMakes.forEach(([make, count]) => {
    console.log(`   ${make}: ${count} unique models`);
  });
}

deduplicateCarsByModel().catch(error => {
